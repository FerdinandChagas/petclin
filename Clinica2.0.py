...
class Animal:
    def __init__(self, nome, idade, especie, raca, porte, dono_cpf, matricula):
        self.nome = nome
        self.idade = idade
...
class Vacina(Medicamento):
    def __init__(self, nome, fabricante, validade, valor, situacao):
...
def cadastrar_animal(clinica):
    ...
    if cliente is None:
        print(
            f"\nCliente com o CPF: {dono_cpf} não encontrado. Por favor, cadastre o cliente antes de associar o animal.")
        return

    animal = Animal(nome, idade, especie, raca, porte,
                    dono_cpf, matricula=len(cliente.animais) + 1)
...
def cadastrar_funcionario(clinica):
    ...
    funcionario = Funcionario(nome, cpf, telefone, email, endereco, data_cadastro, salario,
                              funcao, horario, escala, login, senha, matricula=len(clinica.funcionários)+1)
    ...
    if len(clinica.clientes) == 0:
        ...
        for cliente in clinica.clientes:
            ...
            for animal in cliente.animais:
                print(
                    f"Nome: {animal.nome} | Vacinas: {animal.vacinas.situacao}")
            print('{:=^110}'.format(''))
...
def listar_funcionarios(clinica):
    if len(clinica.funcionarios) == 0:
        ...
        for funcionario in clinica.funcionarios:
            print(
                f"Matrícula: {funcionario.matricula} | Nome: {funcionario.nome} | CPF: {funcionario.cpf} | Função: {funcionario.funcao}\n")
        print('{:=^110}'.format(''))
...
def cancelar_agendamento(clinica):
    ...
    print(
        f"\nAgendamento para {a.animal.nome} no dia {consulta.data} às {consulta.hora} cancelado.")
...
def consultar_pagamento(clinica):
